...
1. Function/method docstrings follow this format:

#####################################################################################
    def my_function(param1:Type, param2:Type=default, ...) -> ReturnType:
        \'''
        Brief explanation here. Order of docstring is shown below.
        Descriptions are omitted in the case that none is required.

        Types/defaults should be in the signature. Use the `typing`
        module. `__future__.annotations` is imported for forward
        references.

        Parameters
        ----------
            param1 : Explanation. Dummy parameters should be explained.
                - Further explanation if needed.
                  Subsequent lines on same indentation level.
                - ...
            ...
        
        Returns
        -------
            type : Explanation. If None, no return description. If tuple:
                - Explain objects in tuple.
                  Subsequent lines on same indentation level.
                - ...
        
        Raises
        ------
            ErrorType : How/why error occurs, only if raised by this file.
                - Further explanation if needed.
                  Subsequent lines on same indentation level.
                - ...
            ...
        ...
        References
        ----------
            List any outside references if needed.
        
        Notes
        -----
            Anything that the user may need to take note of when using the function.
        
        Credits
        -------
            If credit is to be attributed to anyone.
        \'''
...
2. If the function/method is simple/self-explanatory, a one-line docstring is sufficient. 

3. Descriptive comments should be left on/near ambiguous expressions/variables. 
    - If doing a single-line comment after an expression, it should be 2 spaces after.
      Example:

      ``` var = expression  # explanatory comment ```

4. Expressions/algorithms that could see some improvement should have a comment.

5. Strange code that is required to prevent breaking of the program should be noted.

6. Use ndarrays as much as possible. For-loops are discouraged. If building a list
   with one, use a list comprehension.
'''